...
`fn` is read from the latest commit. Options use required `delay`, optional `leading`, `trailing`, and finite `maxWait`.
...
Defaults are trailing-only. `flush` clears both timers and returns the callback result; unmount clears pending work without invoking it.
