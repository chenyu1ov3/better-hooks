...
Failed reads keep the current value and report the error. Failed writes do not replace the current value. `remove` restores the captured initial value. Subscribers for a key share a store.
